...
# print(f"Final sample: {len(df_clean)} obs | {df_clean['symbol'].nunique()} firms")
# # ─── RUN MODELS ───
...
# ─── DROP MISSING ───
reg_vars = ['car_01', 'stress_qa', 'stress_pr', 'vol', 'mom',
            'lnmve', 'bm', 'UE', 'POSWORDS', 'NEGWORDS']
...
# Model 1 — No FE, two-way clustered SE
m1 = pf.feols(
    'car_01 ~ stress_qa + stress_pr + vol + mom + lnmve + bm + UE + POSWORDS + NEGWORDS',
    data=df_clean,
    vcov={'CRV1': 'symbol + time'}
)
print(f"model 1 cluster no FE result {m1.summary()}")   
...
# Model 2 — Firm FE only
m2 = pf.feols(
    'car_01 ~ stress_qa + stress_pr + vol + mom + lnmve + bm + UE + POSWORDS + NEGWORDS | symbol',
    data=df_clean,
    vcov={'CRV1': 'symbol + time'}
)
print(f"model 2 firm FE result {m2.summary()}")
...
# Model 3 — Time FE only
m3 = pf.feols(
    'car_01 ~ stress_qa + stress_pr + vol + mom + lnmve + bm + UE + POSWORDS + NEGWORDS | time',
    data=df_clean,
    vcov={'CRV1': 'symbol + time'}
)
print(f"model 3 time FE result {m3.summary()}")
# Model 4 — Firm + Time FE
m4 = pf.feols(
    'car_01 ~ stress_qa + stress_pr + vol + mom + lnmve + bm + UE + POSWORDS + NEGWORDS | symbol + time',
    data=df_clean,
    vcov={'CRV1': 'symbol + time'}
)
print(f"model 4 firm + time FE result {m4.summary()}")
