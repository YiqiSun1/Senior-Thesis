...
# ─── LOAD DATA ───
master = pd.read_csv('Data/master_final_car2180.csv')
...
for idx, row in master.iterrows():
    ...
    if len(after_day2) >= 30:
        day30 = after_day2.iloc[29]['date']
    else: # if there are fewer than 30 trading days after day 2, use the last available trading day
        day30 = after_day2.iloc[-1]['date'] if not after_day2.empty else None
    ...
    # Get window
    window = firm[(firm['date'] >= day2) & (firm['date'] <= day30)]

    if window.empty:
        results.append({'symbol': symbol, 'call_date': call_date, 'car_230': np.nan})
        continue
    ...
    car_230 = stock_compound - market_compound
...
print(f"\nMissing car_230: {master['car_230'].isna().sum()}")
print(f"\nCAR(2,30) summary:")
print(master['car_230'].describe())
master.to_csv('Data/master_final_car230.csv',      index=False)
print("\n✅ Saved")